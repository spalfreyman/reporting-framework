...
const syncInto = (
  appDir,
  { check = false, withNode = false, bundlerImports = false } = {}
) => {
  ...
  const actual = fs.existsSync(hashFile)
    ? fs.readFileSync(hashFile, 'utf8').trim()
    : null;

  const present =
    fs.existsSync(dest) && (!includeNode || fs.existsSync(nodeDest));
...
const appsFromConnectYaml = (yamlPath) => {
  ...
  return [...text.matchAll(/^\s*-\s+name:\s*(\S+)/gm)].map((m) =>
    m[1].replace(/['"]/g, '')
  );
};
...
const discoverAppDirs = () => {
  const dirs = [];
  for (const name of appsFromConnectYaml(
    path.join(REPO_ROOT, 'connect.yaml')
  )) {
  ...
  if (fs.existsSync(connectorsRoot)) {
    for (const connector of fs.readdirSync(connectorsRoot)) {
      ...
      for (const name of appsFromConnectYaml(
        path.join(connectorDir, 'connect.yaml')
      )) {
...
if (targets.length === 0) {
  if (isAtRepoRoot && !all && !check) {
    console.log(
      'run with --all from the repo root, or from inside an app folder'
    );
  } else {
