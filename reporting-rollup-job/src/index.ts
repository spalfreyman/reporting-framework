...
const JOB_NAME = 'reporting-rollup-job';
...
// Only run when invoked directly, so tests can import runJob without launching it.
    ...
    .then((result) => {
      if (!result.skipped) {
        process.stdout.write(
          `${JSON.stringify({ level: 'info', message: 'done', ...result })}\n`
        );
      }
      process.exit(0);
    })
