...
const schema = z
  .object({
    ...
    SOURCE_ID: z.string().regex(/^[a-z0-9-]+$/).default('warehouse'),
    SOURCE_DISPLAY_NAME: z.string().default('Data warehouse'),
    WAREHOUSE_KIND: z.enum(['postgres', 'bigquery', 'snowflake']).default('postgres'),
    MODE: z.enum(['live', 'demo']).default('demo'),
    WAREHOUSE_TIMEZONE: z.string().default('UTC'),
  ...
  .refine((c) => c.MODE !== 'live' || c.WAREHOUSE_KIND !== 'postgres' || c.PG_CONNECTION_STRING, {
    message: 'MODE=live with WAREHOUSE_KIND=postgres requires PG_CONNECTION_STRING',
  });
