...
/** postDeploy: publish the ERP/OMS descriptor so the gateway discovers it. Idempotent. */
const main = async (): Promise<void> => {
  ...
  process.stdout.write(
    `${JSON.stringify({ level: 'info', message: `descriptor ${outcome.action}`, sourceId: outcome.sourceId, mode: config.MODE })}\n`
  );
};
main().catch((error) => {
